...
function loadDaily(){const base={focusSeconds:0,focusStartedAt:null,toiletSeconds:0,toiletStartedAt:null,flow:0,workSkill:0,lifeSkill:0,mask:0,joy:0,coffee:0,sarcasm:0,dad:0},saved=JSON.parse(localStorage.getItem('banpetDaily')||'null');return saved?.date===todayKey()?{...saved,events:{...base,...saved.events}}:{date:todayKey(),frustration:0,meetingSeconds:0,meetingStartedAt:null,events:base}}
let daily=loadDaily();
...
function toiletSeconds(){return daily.events.toiletSeconds+(daily.events.toiletStartedAt?Math.max(0,(Date.now()-daily.events.toiletStartedAt)/1000):0)}
function verdict(){const e=daily.events;if(e.dad>=3)return['今日老登','输出很多，听取意见很少。'];if(e.sarcasm>=3)return['语言艺术家','没有直接说，但该懂的都懂了。'];if(daily.frustration>=6)return['忍者神龟','今天的主要技能是忍住。'];if(e.mask>=4)return['奥斯卡在逃影后','表面配合，内心已经下班。'];if(focusSeconds()>=7200)return['心流战神','今天真的做成了一点东西。'];if(e.joy>=3)return['快乐漏网之鱼','工作居然没能完全毁掉今天。'];if(e.coffee>=3)return['液体工位','血液里可能已经没有血了。'];return['平静打工人','今日暂未检测到明显班味。']}
function updateDailyTools(){if(daily.date!==todayKey())daily=loadDaily();const seconds=meetingSeconds(),meetingEarned=hourlyPay()*seconds/3600;$('#meetingTime').textContent=clock(seconds);$('#meetingMoney').textContent=moneyValue(meetingEarned);$('#frustrationCount').textContent=daily.frustration;$('#frustrationCaption').textContent=daily.frustration===0?'目前情绪尚可，继续观察。':daily.frustration<3?'班味上升，宠物已经注意到了。':daily.frustration<6?'精神损伤明确，公司继续计费。':'建议立即下班，至少心理上。';$('#meetingButton').textContent=daily.meetingStartedAt?'结束这场会':'开始无效会议';$('#meetingButton').classList.toggle('active',Boolean(daily.meetingStartedAt));$('#focusValue').textContent=daily.events.focusStartedAt?`${clock(focusSeconds())} · 结束`:(daily.events.focusSeconds?clock(daily.events.focusSeconds):'开始计时');$('#toiletValue').textContent=daily.events.toiletStartedAt?`${clock(toiletSeconds())} · 结束`:(daily.events.toiletSeconds?`${clock(daily.events.toiletSeconds)} · ${moneyValue(hourlyPay()*daily.events.toiletSeconds/3600)}`:'开始计时');['flow','workSkill','lifeSkill','mask','joy','coffee','sarcasm','dad'].forEach(k=>$(`#${k}Value`).textContent=`${daily.events[k]} 次`);const [title,line]=verdict();$('#dailyVerdict').textContent=title;$('#dailyVerdictLine').textContent=line}
function archiveToday(){const archive=JSON.parse(localStorage.getItem('banpetArchive')||'{}');archive[todayKey()]={earned:latestEarned,meetingSeconds:meetingSeconds(),frustration:daily.frustration,currency:$('#currency').value,pet:currentPet.id,events:{...daily.events,focusSeconds:focusSeconds(),focusStartedAt:null,toiletSeconds:toiletSeconds(),toiletStartedAt:null}};localStorage.setItem('banpetArchive',JSON.stringify(archive))}
...
document.querySelectorAll('[data-event]').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.event;if(key==='focus'){if(daily.events.focusStartedAt){daily.events.focusSeconds=focusSeconds();daily.events.focusStartedAt=null;$('#speech').textContent='专注结束。你真的做成了一点东西。'}else{daily.events.focusStartedAt=Date.now();$('#speech').textContent='进入专注。班宠替你挡住一点噪音。'}}else if(key==='toilet'){if(daily.events.toiletStartedAt){daily.events.toiletSeconds=toiletSeconds();daily.events.toiletStartedAt=null;$('#speech').textContent=`本次带薪如厕赚到 ${moneyValue(hourlyPay()*daily.events.toiletSeconds/3600)}。`}else{daily.events.toiletStartedAt=Date.now();$('#speech').textContent='带薪如厕开始，公司继续计费。'}}else{daily.events[key]+=1;const lines={flow:'心流捕获成功。今天不只是熬过去。',workSkill:'工作技能 +1，至少这班没有白上。',lifeSkill:'兴趣技能 +1，下班后的你也在长大。',mask:'假面营业已记录。表面配合，内心清醒。',joy:'开心时刻已入账。这个比工资珍贵。',coffee:'咖啡到账，精神暂时续费。',sarcasm:'语言艺术已记录。该懂的都懂了。',dad:'爹味输出达成，请适量使用。'};$('#speech').textContent=lines[key]}saveDaily();updateDailyTools()}));
...
function fillSummary(){const seconds=meetingSeconds(),meetingEarned=hourlyPay()*seconds/3600;$('#summaryDate').textContent=new Date().toLocaleDateString('zh-CN',{month:'long',day:'numeric',weekday:'short'});$('#summaryPet').textContent=currentPet.emoji;$('#summaryEarned').textContent=moneyValue(latestEarned);$('#summaryMeeting').textContent=moneyValue(meetingEarned);$('#summaryMeetingTime').textContent=clock(seconds);$('#summaryFrustration').textContent=`${daily.frustration} 次`;const [title,line]=verdict();$('#summaryVerdict').textContent=`“${title}：${line}”`}
